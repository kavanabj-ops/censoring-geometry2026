# -*- coding: utf-8 -*-
# 65_n_sensitivity.py - n 门槛敏感性：insufficient 比例 与 X% 随 min_n 变化
...
def survey(drugs, genes_fn, min_n):
    insuff = 0
    suff = 0
    nonid = 0
    for drug in drugs:
...
print(f"\n=== n 门槛敏感性（c*={C_STAR}）===")
print(f"{'min_n':>6} | {'靶向 insuff':>14} {'X%':>8} | {'细胞毒 insuff':>14} {'X%':>8}")
for mn in [3, 5, 10, 15]:
    it, st_, nid = survey(targeted_drugs, lambda d: parse_genes(drug_target[d]), mn)
    ic, sc, nidc = survey(cyto_drugs, lambda d: DRIVER_GENES, mn)
    xt = nid/max(st_,1); xc = nidc/max(sc,1)
    it_pct = it/max(it+st_,1); ic_pct = ic/max(ic+sc,1)
    print(f"{mn:>6} | {it_pct:>13.1%} {xt:>8.1%} | {ic_pct:>13.1%} {xc:>8.1%}  (靶向 suff={st_}, 细胞毒 suff={sc})")
